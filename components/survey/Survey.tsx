...
const Survey = ({ props }: any) => {
        ...
        if (counter + 1 <= props?.survey?.questions.length) {
            timer = setTimeout(() => {
                setAnswers([
                    ...answers,
                    {
                        question: props.survey.questions[counter].text,
                        answer: null,
                    },
                ]);
                setCounter(counter + 1);
            }, props.survey.questions[counter].lifetimeSeconds * 1000);
        }
    }, [counter]);
            ...
            return (
                <div>
                    ...
                    <div className={styles.container__questions__options}>
                        {props.survey.questions[counter].options.map((o, y) => {
                            return (
                                <Button
                                    key={y}
                                    block
    ...
    const handleTransfer = async () => {
        ...
        try {
            const submitData = await erc20.submit(
                ethers.utils.parseUnits('0.000000000000000001', 18),
                answerArrayUnits
            );
            if (submitData) {
